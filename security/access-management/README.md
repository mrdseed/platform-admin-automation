...
Pipeline principals are scoped per workload subscription; state access is limited to `sub-acme-management`. See the `subscriptions:` and `service_principals:` blocks in [users-groups.yml](users-groups.yml) and [Platform Governance](../../docs/platform-governance.md#3-multi-subscription-governance).
